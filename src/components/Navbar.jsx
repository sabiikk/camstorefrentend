...
//   const userData = JSON.parse(sessionStorage.getItem("loggeduser"));
  ...
  return (
    <>
      <header className="fixed top-0 left-0 w-full z-50 bg-red-600">
        <nav className="max-w-screen-2xl mx-auto px-4 py-6 flex justify-between items-center">
          {/* left side */}
          <div className="flex items-center md:gap-16 gap-4">
            <Link to="/">
              <HiMiniBars3CenterLeft style={{ color: "white" }} className="size-6" />
            </Link>
          ...
          <div className="relative flex items-center md:space-x-3 space-x-2">
            <div>
              {currentUser ? (
                ...
                <Link to="/login">
                  <HiOutlineUser style={{ color: "white" }} className="size-6" />
                </Link>
              )}
            </div>
            ...
            <Link
              style={{ color: "red" }}
              to="/cart"
              className="bg-white p-1 sm:px-6 px-2 flex items-center rounded-sm"
            >
